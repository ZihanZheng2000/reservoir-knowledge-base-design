...
def extract_pdf(path: Path) -> str:
    try:
        ...
        return f"[PDF text extraction skipped: pypdf unavailable: {exc}]"
    try:
    ...
    for src in load_manifest(args.manifest):
        ...
        try:
            ...
            if status == "ok" and len(extracted.strip()) < MIN_EXTRACTED_TEXT_CHARS:
                ...
                content_quality_note = f"Extracted text is short ({len(extracted.strip())} chars). Short content can still be valid, but it should be reviewed before KU extraction."
